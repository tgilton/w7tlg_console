#!/usr/bin/env python3
"""Inventory the receiver panels' BOTTOM ROW (stage 4b scope): the gains
and mode cards, i.e. from `.vol-alc-group`/`.v2-rx-bottom` to
`.scope-group`, which is the spectrum and out of scope.

    ./venv/bin/python ui-redesign/rx_bottomrow_inventory.py [path]
"""
...
def main():
    ...
    jsc = js_classes(js)
    marker = '<div class="v2-rx-bottom">' if 'v2-rx-bottom' in src else '<div class="vol-alc-group">'
    pos = 0
    for label in ("RX1", "RX2"):
        a = src.index(marker, pos)
        b = src.index('<div class="scope-group">', a)
        pos = b
        reg = src[a:b]
        ...
        print(f"== {label} BOTTOM ROW ==")
        for title, items in (("IDS", ids), ("HANDLERS", handlers), ("DATA ATTRS", datas),
                             ("JS-REFERENCED CLASSES", sorted(present & jsc))):
