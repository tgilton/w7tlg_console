#!/usr/bin/env python3
"""Inventory everything under the receiver panels' gains/mode row
(stage 5 scope): scope card, display controls, tuning, filter/AGC-NR, EQ.

Region: from `.scope-group` to the end of the panel.

    ./venv/bin/python ui-redesign/rx_lower_inventory.py [path]
"""
...
def main():
    ...
    jsc = js_classes(js)
    ends = ('<!-- ── 4.', '<!-- ── 4b.', '<div class="panel panel-center2"')
    for label, panel in (("RX1", 'class="panel panel-center"'),
                         ("RX2", 'class="panel panel-center2"')):
        a = src.index('<div class="scope-group">', src.index(panel))
        cands = [src.index(e, a) for e in ends if e in src[a:]]
        b = min(cands) if cands else len(src)
        reg = src[a:b]
